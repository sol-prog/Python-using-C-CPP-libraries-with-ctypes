...
struct BMP {
    ...
    void read(const char *fname) {
        ...
        if (inp) {
            ...
            if (bmp_info_header.width % 4 == 0) {
                ...
                file_header.file_size += (uint32_t)data.size() + bmp_info_header.height * (uint32_t)padding_row.size();
            }
        }
        else {
    ...
    BMP(int32_t width, int32_t height, bool has_alpha = true) {
        ...
        if (has_alpha) {
            ...
            uint32_t new_stride = make_stride_aligned(4);
            file_header.file_size = file_header.offset_data + (uint32_t)data.size() + bmp_info_header.height * (new_stride - row_stride);
        }
    }
